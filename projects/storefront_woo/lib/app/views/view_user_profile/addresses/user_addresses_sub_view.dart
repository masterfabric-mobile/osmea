...
class _UserAddressesViewState extends State<UserAddressesView> {
  ...
  Future<void> _loadAddresses() async {
    ...
    try {
      ...
      if (cachedAddresses != null) {
        debugPrint('✅ [ADDRESS CACHE] Loaded ${cachedAddresses.length} addresses from cache');
      ...
      debugPrint('⚠️ [ADDRESS CACHE] No cache found, loading addresses from API...');
      final savedResponse = await _usersManagerService.getUserAddresses();
      
      debugPrint('✅ [ADDRESS CACHE] API Response: ${savedResponse.addresses.length} addresses received');
      ...
      if (cachedAddresses != null) {
        debugPrint('✅ [ADDRESS CACHE] Using cached addresses as fallback (${cachedAddresses.length} addresses)');
        setState(() {
          _savedAddresses = cachedAddresses;
          _isLoading = false;
        });
  ...
  Future<List<UserAddress>?> _loadAddressesFromCache() async {
    try {
      ...
      final cachedJson = await storage.getItem('user_addresses_cache');
      final timestamp = await storage.getItem('user_addresses_cache_timestamp');
  ...
  Future<List<UserAddress>?> _loadOrderAddressesFromCache() async {
    try {
      final storage = LocalStorageHelper();
      final cachedOrdersJson = await storage.getItem('user_orders_cache');
      final timestamp = await storage.getItem('user_orders_cache_timestamp');
  ...
  List<UserAddress> _extractAddressesFromOrders(List<DetailedUserOrder> orders) {
    ...
    debugPrint('   ✅ Extracted ${uniqueOrderAddresses.length} unique addresses from ${orders.length} orders');
  ...
  bool _shouldRefreshCache(String? timestamp, {Duration maxAge = const Duration(minutes: 10)}) {
    ...
    try {
      ...
      if (age > maxAge) {
        ...
        debugPrint('   ✅ Cache is fresh (${age.inMinutes} minutes old), skipping refresh');
        return false;
      }
    } catch (e) {
  ...
  Future<void> _refreshAddressesFromApi() async {
    try {
      ...
      if (!_shouldRefreshCache(timestamp)) {
        debugPrint('⏭️ [ADDRESS CACHE] Skipping refresh - cache is still fresh');
        return;
      }
      
      debugPrint('🔄 [ADDRESS CACHE] Refreshing addresses from API in background...');
      final savedResponse = await _usersManagerService.getUserAddresses();
      await _saveAddressesToCache(savedResponse.addresses);
      
      debugPrint('✅ [ADDRESS CACHE] Addresses refreshed from API (${savedResponse.addresses.length} addresses)');
  ...
  Future<void> _loadOrderAddresses() async {
    try {
      ...
      if (cachedOrderAddresses != null) {
        debugPrint('✅ [ORDER CACHE] Loaded ${cachedOrderAddresses.length} order addresses from cache');
  ...
  Future<void> _saveOrdersToCache(List<DetailedUserOrder> orders) async {
    try {
      ...
      final timestamp = DateTime.now().toIso8601String();
      await storage.setItem('user_orders_cache_timestamp', timestamp);
  ...
  Future<void> _refreshOrderAddressesFromApi() async {
    try {
      ...
      debugPrint('🔄 [ORDER CACHE] Refreshing orders from API in background...');
      ...
      debugPrint('✅ [ORDER CACHE] Orders refreshed from API (${detailedOrders.length} orders, ${addresses.length} addresses)');
    } catch (e) {
