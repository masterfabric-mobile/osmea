...
class RemoveItemHandler implements ApiRequestHandler {
  @override
  Future<Map<String, dynamic>> handleRequest(
    ...
    if (method != 'POST') {
      return {
        "status": "error",
        "message": "Method $method not supported for Remove Item API",
    ...
    try {
      ...
      debugPrint('🗑️ Starting remove item from cart:');
      ...
      // Call the service
      final response = await cartService.removeItem(
      ...
      debugPrint('🔍 Remove Item Response Debug:');
      ...
      if (response.errors != null && response.errors!.isNotEmpty) {
        debugPrint('❌ Remove item failed with errors: ${response.errors}');
      ...
      debugPrint("🚨 Remove Item Error Details: $e");

      String errorMessage = "Failed to remove item from cart: ${e.toString()}";
      ...
      if (e.toString().contains('DioException') ||
        ...
        if (e.toString().contains('400')) {
          ...
          errorMessage = "Invalid request. Please check your item key";
          ...
          errorMessage = "Invalid item key or cart state";
        } else {
          errorMessage = "Network error occurred while removing item from cart";
        }
  ...
  Map<String, List<ApiField>> get requiredFields => {
        'POST': [
          const ApiField(
            name: 'key',
            label: 'Cart Item Key',
            hint: 'The key of the cart item to remove (get from cart items list)',
            isRequired: true,
          ),
          const ApiField(
            name: 'cart_token',
