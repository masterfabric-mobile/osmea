...
class CreatePriceRuleDiscountOrderHandler implements ApiRequestHandler {
  @override
  Future<Map<String, dynamic>> handleRequest(
    ...
    switch (method) {
      case 'POST':
        ...
        try {
          final model = create_request.CreatePriceRuleDiscountOrderRequest(
          ...
          final CreatePriceRuleDiscountOrderResponse response = await GetIt.I
              .get<PriceRuleService>()
              .createPriceRuleDiscountOrder(
          ...
          return {
            "status": "success",
            "message": "Price rule for order discount created successfully",
          ...
          return {
            "status": "error",
            "message":
                "Failed to create price rule for order discount: ${e.toString()}",
            "timestamp": DateTime.now().toIso8601String(),
          };
        }
  ...
  Map<String, List<ApiField>> get requiredFields => {
        'POST': [
          ...
          const ApiField(
            name: 'target_type',
            label: 'Target Type',
            hint: 'Enter the target type (e.g., order)',
          ),
          const ApiField(
          ...
          const ApiField(
            name: 'value_type',
            label: 'Value Type',
            hint: 'Enter the value type (e.g., fixed_amount, percentage)',
          ),
          const ApiField(
            name: 'value',
            label: 'Value',
            hint:
                'Enter the discount value (e.g., -10.0 for fixed amount, 10.0 for percentage)',
          ),
          const ApiField(
