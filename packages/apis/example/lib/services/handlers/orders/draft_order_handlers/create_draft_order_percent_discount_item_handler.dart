...
class CreateDraftOrderPercentDiscountItemHandler implements ApiRequestHandler {
  @override
  Future<Map<String, dynamic>> handleRequest(
    ...
    switch (method) {
      case 'POST':
        ...
        final String discountValueType = 'percentage';
        ...
        if (draftOrderId == null ||
          ...
          return {
            "status": "error",
            "message":
                "Draft Order ID, Line Item Title, Price, Quantity, Discount Description, and Discount Value are required.",
        ...
        try {
          final model =
              create_request.CreateDraftOrderPercentDiscountItemRequest(
          ...
          final CreateDraftOrderPercentDiscountItemResponse response =
              await GetIt.I
          ...
          return {
            "status": "success",
            "message":
                "Draft order item with percentage discount created successfully",
            "draft_order": response.draftOrder?.toJson(),
            "timestamp": DateTime.now().toIso8601String(),
          };
        } catch (e) {
          print("Error creating draft order item with percentage discount: $e");
          return {
            "status": "error",
            "message":
                "Failed to create draft order item with percentage discount: ${e.toString()}",
            "timestamp": DateTime.now().toIso8601String(),
          };
        }
  ...
  Map<String, List<ApiField>> get requiredFields => {
        'POST': [
          ...
          const ApiField(
            ...
            hint: 'Enter the percentage discount value (e.g., 10.0)',
          ),
        ],
      };
}
