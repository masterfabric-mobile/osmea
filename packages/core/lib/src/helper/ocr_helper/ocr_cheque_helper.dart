...
class OcrChequeHelper {
  ...
  static Future<Map<String, dynamic>> readChequeInfo(String imagePath) async {
    ...
    final fields = extractChequeFields(ocrText);
    final isCheque = isChequeText(ocrText, fields);
    return {
      'rawText': ocrText,
      'documentType': isCheque ? 'cheque' : 'other',
      ...fields,
    };
  }
  ...
  static Map<String, String?> extractChequeFields(String text) {
  ...
  /// Checks if the given OCR text contains cheque-related keywords and fields
  static bool isChequeText(String text,
      [Map<String, dynamic>? extractedFields]) {
    if (text.toLowerCase().contains('http') ||
        text.toLowerCase().contains('ftp') ||
        text.toLowerCase().contains('mailto') ||
        text.toLowerCase().contains('git@') ||
        text.toLowerCase().contains('tel:') ||
        text.toLowerCase().contains('urn:') ||
        RegExp(r'[\w\.-]+@[\w\.-]+\.\w{2,4}').hasMatch(text) ||
        RegExp(r'\+\d{1,3}-\d{3}-\d{3}-\d{4}').hasMatch(text)) {
      return false;
    }
    ...
    if (extractedFields != null) {
      ...
      for (final key in fieldsToCheck) {
        final val = extractedFields[key];
        if (val != null &&
            val.toString().isNotEmpty &&
            val.toString() != '[Bulunamadı]') {
          print(
              '✅ Alanlardan biri bulundu ($key: $val), çek olarak kabul edildi!');
          return true;
        }
      }
    }

    // Anahtar kelime fallback (sadece Türkçe ve çek ile ilgili varyasyonlar kritik)
    final chequeKeywords = [
      'çek',
      'cheque',
      'çek',
      'çeque',
    ...
    int matchCount = chequeKeywords.where((k) => lowerText.contains(k)).length;

    // Türkçe varyasyonlardan biri varsa, eski mantık devam etsin
    final hasTurkishCheque = ['çek', 'cheque', 'çek', 'çeque', 'cek']
        .any((k) => lowerText.contains(k));

    // Eğer 'check' kelimesi geçiyorsa, başka bir anahtar kelime de aranmalı
    if (lowerText.contains('check')) {
      // 'check' dışında başka anahtar kelime var mı?
      final otherKeywords = chequeKeywords.where((k) => k != 'check');
      final hasOtherKeyword = otherKeywords.any((k) => lowerText.contains(k));
      if (!hasOtherKeyword) {
        print(
            '⚠️ Sadece İngilizce check kelimesi geçti, çek olarak kabul edilmedi!');
        return false;
      }
      // Eğer başka anahtar kelime varsa, çek olarak kabul edilsin
      print('✅ check + başka anahtar kelime var, çek olarak kabul edildi!');
      return true;
    }

    if (hasTurkishCheque) {
      print('✅ Türkçe çek anahtar kelimesi bulundu, çek olarak kabul edildi!');
      return true;
    }

    print('🔍 Çek anahtar kelime eşleşme sayısı: $matchCount');
    return matchCount >= 2;
  }

  /// Web: Analyze a batch of base64 images, check if each is a cheque, and extract fields
  static Future<List<Map<String, dynamic>>> analyzeImagesWeb(
    ...
    for (final base64 in base64Images) {
      final ocrResult = await readChequeInfoOptimized(base64);
      final isCheque = isChequeText(ocrResult['rawText'] ?? '', ocrResult);
      results.add({
        'isCheque': isCheque,
        'fields': ocrResult,
      });
